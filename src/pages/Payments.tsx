...
export default function Payments() {
  ...
  // Set of booking IDs that have at least one SUCCESS payment on this page
  const successBookingIds = new Set(
    items.filter(i => i.status === 'SUCCESS').map(i => i.booking_id)
  )

  // Set of booking IDs that have BOTH a SUCCESS AND at least one PENDING on this page
  // Used to show "Cancel Dupes" on the SUCCESS row
  const pendingBookingIds = new Set(
    items.filter(i => i.status === 'PENDING').map(i => i.booking_id)
  )
  const isPendingWithSuccessBookingIds = new Set(
    [...successBookingIds].filter(id => pendingBookingIds.has(id))
  )
              ...
              <table className="data-table">
                ...
                <tbody>
                  {items.length === 0 ? (
                    ...
                    const isPendingWithSuccess = p.status === 'PENDING' && p.booking_id && isPendingWithSuccessBookingIds.has(p.booking_id)
                      ...
                      <tr key={p.id} style={isPendingWithSuccess ? { background: '#FFFBEB' } : isDuplicateSuccess ? { background: '#FEF2F2' } : {}}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                            ...
                            {/* SUCCESS: Cancel all PENDING dupes for this booking in one click */}
                            {p.status === 'SUCCESS' && isPendingWithSuccessBookingIds.has(p.booking_id) && (
                              <button onClick={() => openCancelDupes(p)}
                                style={{ fontSize: 11, color: '#7C3AED', background: '#F3E8FF', border: '1px solid #C4B5FD', padding: '3px 8px', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>
                                🧹 Cancel Dupes
                              </button>
                            )}
                            ...
                            {/* PENDING: Cancel this single record */}
                            {p.status === 'PENDING' && (
                            ...
                            {/* PENDING with SUCCESS sibling: Cancel ALL pending for this booking at once */}
                            {isPendingWithSuccess && (() => {
                              const successTx = items.find(i => i.booking_id === p.booking_id && i.status === 'SUCCESS')
                              return successTx ? (
                                <button onClick={() => openCancelDupes(successTx)}
                                  style={{ fontSize: 11, color: '#7C3AED', background: '#F3E8FF', border: '1px solid #C4B5FD', padding: '3px 8px', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>
                                  Cancel All
                                </button>
                              ) : null
